...
class EPS:
    def __init__(self, instructions, special_library={}, path=None, file="eps", extension=".eps"):
        # nplibs holds dictionaries like this
        # "lib_name": module_class,
        # where module_class is the class object of that library
        
        # CONFIGURATIONS
        self.version = "1.0.5" # current version
    ...
    def expression(self, exp):
        ...
        """
        Converts expressions written in epsilon into a AST readable form that can be run in self.eval or instantly to evaluation
        ...
        - arbitrary epsilon expressions (like built ins)
        """
        iseval = False
        ...
        for args in arguments:
            if args.startswith(("(", "!(")):
                ...
                if expression["iseval"]: # instant evaluation, use eval() since there is no more arb code
                    boolean = eval(str(expression["expr"]), {}, self.variables)
                else:
    ...
    def execute_functions(self, instruction):
        ...
        if not instruction:
            ...
            # EPSILON SOURCE CODE DEVELOPER DEBUGGING
        ...
        # error handling keywords
            ...
            for lib in args:
                ...
                if lib in self.libraries:
                    ...
                    continue
                if Path(self.path / Path(lib.replace(".", "/")).with_suffix(".eps")).exists():
                    # execute .eps and get variables, functions, and methods
                    # types of imports -
                    # import file_eps <- imports full code
                    # import directory.file_eps <- imports code
                    # import file_eps get func_or_class <- imports a function or class instead of the whole code
                    types = "dir" if "." in lib else "cwd"
                    path = str(self.path)
                    if types == "dir":
                        # get directory
                        pathp = path / Path(lib.replace(".", "/")).with_suffix(".eps")
                        pathx = path / Path(lib.replace(".", "/")).with_suffix(".nxx")
                    else:
                        pathp = path / Path(lib).with_suffix(".eps")
                    ...
                    # execute code
                    eps = EPS(code, {}, absolute_path, name)
                    result = eps.execute()
                    variables = {}
                    functions = {}
                    func_scope = {}
                    classes = {}
                    name = name.removesuffix(".eps")
                    # get variables, functions, classes, and libraries
                    if has_get and get_val is not None:
                        value = get_val.split(",")
                        for get_val in value:
                            get_val = get_val.strip()
                            if get_val in eps.variables.keys():
                                variables[get_val] = eps.variables[get_val]
                            if get_val in eps.functions.keys():
                                functions[get_val] = eps.functions[get_val]
                            if get_val in eps.classes.keys():
                                classes[get_val] = eps.classes[get_val]
                    else:
                        for v in eps.variables.keys():
                            variables[name + "." + v] = eps.variables[v]
                        for f in eps.functions.keys():
                            functions[name + "." + f] = eps.functions[f]
                        for fs in eps.func_scope.keys():
                            func_scope[name + "." + f] = eps.func_scope[f]
                        for c in eps.classes.keys():
                            classes[name + "." + c] = eps.classes[c]
                    if eps.libraries:
                        self.library.extend(eps.library)
                        self.library_name.update(eps.library_name)
                        self.name_library.update(eps.name_library)
                    if eps.nplibs != {}:
                        self.nplibs.update(eps.nplibs)
                        self.nplibs_acc.update(eps.nplibs_acc)
        ...
        def built_in_functions(left, main, right, method):
            ...
            if True:
                ...
                # handles classes
                    ...
                    self.objects[left] = {}
                    self.objects[left]["variables"] = self.classes[name]["variables"].copy()
